...
Read `.go-guardian/session-findings.md` to check what the reviewer and security agents have flagged in this session — write tests that specifically target those findings (e.g. race conditions, error paths, edge cases). The file is regenerated by `go-guardian scan --patterns`, so it reflects the current session's cross-agent view.
